...
struct test_flash_attn_ext : public test_case {
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        if (mask) {
            m = compact_mask ? ggml_new_tensor_4d(ctx, GGML_TYPE_I32, 4, kv + nb, 1, nr23[1]) : ggml_new_tensor_4d(ctx, GGML_TYPE_F16, kv, nb, 1, nr23[1]);
            ggml_set_name(m, "m");
        }
    ...
    void initialize_tensors(ggml_context * ctx) override {
        for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != NULL; t = ggml_get_next_tensor(ctx, t)) {
            if (strcmp(t->name, "indices") == 0) {
                ...
                if (compact_mask) {
                    std::vector<int32_t> data(ggml_nelements(t));
                    for (int64_t seq = 0; seq < nr23[1]; ++seq) {
                        int32_t * dst = data.data() + 4*(kv + nb)*seq;
                        for (int64_t j = 0; j < kv; ++j) {
                            const uint32_t bits = j == 0 ? 0x80000003u : j%11 == 0 ? 0 : j%3 == 2 ? 0x80000000u : 1u << (j%3);
                            dst[4*j] = j == 0 ? -1 : (j*37 + seq*3)%kv;
                            memcpy(dst + 4*j + 1, &bits, sizeof(bits));
                            dst[4*j + 2] = j%3;
                            dst[4*j + 3] = j%5;
                        }
                        for (int64_t i = 0; i < nb; ++i) {
                            const uint32_t bits = i%3 == 2 ? 0x80000000u : 1u << (i%3);
                            dst[4*(kv + i)] = (i*23 + 31)%kv;
                            memcpy(dst + 4*(kv + i) + 1, &bits, sizeof(bits));
                            dst[4*(kv + i) + 2] = i%4;
                            dst[4*(kv + i) + 3] = i%6;
                        }
                    }
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    for (ggml_type type : {GGML_TYPE_F16, GGML_TYPE_Q8_0, GGML_TYPE_Q4_0}) {
        for (int64_t nq : {1, 2, 7, 65, 1024}) {
            for (int64_t nk : {512, 1025}) {
                test_cases.emplace_back(new test_flash_attn_ext(256, 256, 2, {6, 1}, nk, nq, true, false, 0, 0, GGML_PREC_F32, type, type, {0, 2, 1, 3}, true, false, 0, true));
            }
        }
    ...
    // Quantized MMA tile loading: partial tiles, strided caches, GQA and multiple sequences.
    for (ggml_type type_KV : {GGML_TYPE_Q8_0, GGML_TYPE_Q4_0}) {
        for (int64_t hs : {64, 96, 128, 256}) {
            test_cases.emplace_back(new test_flash_attn_ext(hs, hs, 2, {3, 2}, 513, 65, true, true, 0, 0, GGML_PREC_F32, type_KV, type_KV, {0, 2, 1, 3}));
            test_cases.emplace_back(new test_flash_attn_ext(hs, hs, 2, {6, 1}, 512, 65, true, false, 0, 0, GGML_PREC_F32, type_KV, type_KV, {0, 2, 1, 3}));
        }
        test_cases.emplace_back(new test_flash_attn_ext(256, 256, 4, {6, 1}, 4096, 65, true, false, 0, 0, GGML_PREC_F32, type_KV, type_KV, {0, 2, 1, 3}));
    }
